...
const ImplicitItem = memo(function ImplicitItem({
  fn,
  gpuAvailable,
  isEditing,
  editExpression,
...
export const ImplicitList: FC = () => {
  ...
  const updateImplicitParamConfig = useAppStore(state => state.updateImplicitParamConfig);
  ...
  if (implicitFunctions.length === 0) {
    return <EmptyState title="暂无隐函数" subtitle="请添加隐函数" />;
  }
      ...
      <ul className="space-y-1.5">
        {implicitFunctions.map(fn => (
          <ImplicitItem
            ...
            onToggleVisibility={() => toggleImplicitVisibility(fn.id)}
            onToggleKeyPoints={() => toggleImplicitKeyPoints(fn.id)}
            onToggleGPU={() => toggleImplicitGPURendering(fn.id)}
            onRemove={() => removeImplicitFunction(fn.id)}
            onStartEdit={() => startEditing(fn)}
            onSaveEdit={() => saveEdit(updateImplicitExpression)}
            onEditExpressionChange={setEditExpression}
            onEditKeyDown={(e) => handleKeyDown(e, updateImplicitExpression)}
            onUpdateParameter={(name, value) => updateImplicitParameter(fn.id, name, value)}
            onUpdateParameterConfig={updateImplicitParamConfig}
            inputRef={inputRef}
          />
        ))}
      </ul>
