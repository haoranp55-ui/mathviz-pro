...
export const ImplicitInput: React.FC = () => {
  ...
  return (
    <div className="p-4 border-b border-gray-700/50 relative" ref={pickerRef}>
      {/* 标题栏 */}
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <div className="relative flex-1">
          <input
            ...
            className="w-full px-3 py-2.5 bg-canvas-panelLight text-white rounded-lg border border-gray-600 focus:border-green-500/50 focus:ring-2 focus:ring-green-500/20 focus:outline-none text-sm placeholder-gray-500 pr-10 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            disabled={!canAdd}
          />
          <button
            type="button"
            ...
            className={`absolute right-2 top-1/2 -translate-y-1/2 px-2 py-1 flex items-center gap-0.5 rounded transition-all ${
              showPicker ? 'text-green-400 bg-green-500/20' : 'text-gray-400 hover:text-white hover:bg-gray-700/50'
            }`}
            title="函数选择器"
            aria-label="函数选择器"
          >
          ...
          {showPicker && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-canvas-panel border border-gray-600 rounded-lg shadow-xl z-20 max-h-64 overflow-y-auto">
              <div className="bg-gradient-to-r from-green-600/20 to-teal-600/20 px-3 py-2 border-b border-gray-600/50">
        ...
        <button
          ...
          className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${
            canAdd && expression.trim()
              ? 'bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-500 hover:to-teal-500 text-white shadow-lg shadow-green-500/20 hover:shadow-green-500/30'
      ...
      <p className="text-xs text-gray-500 mt-2 flex items-center gap-1">
        ...
        <span className="text-green-400 font-mono">a, b, c</span>
      </p>
    </div>
  );
};
