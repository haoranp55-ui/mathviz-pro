...
export const StatusBar: React.FC = () => {
  ...
  return (
    <footer className="h-7 glass border-t border-white/10 flex items-center px-4 text-xs text-gray-400 gap-6 select-none relative z-50">
      {/* 顶部发光线条 */}
      <div 
      ...
      <span className="flex items-center gap-1.5 relative z-10">
        ...
        <span className="text-cyan-400 font-mono font-medium">{zoomPercent}%</span>
      </span>

      {/* 坐标信息 */}
      <span className="flex items-center gap-1.5 relative z-10">
        <div className="w-5 h-5 rounded-md bg-gradient-to-br from-green-500/20 to-teal-500/20 flex items-center justify-center border border-green-500/20">
      ...
      {/* 函数统计 */}
      <span className="flex items-center gap-1.5 relative z-10">
        <div className="w-5 h-5 rounded-md bg-gradient-to-br from-purple-500/20 to-pink-500/20 flex items-center justify-center border border-purple-500/20">
          <svg className="w-3 h-3 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
          </svg>
        </div>
        <span className="text-gray-400">函数</span>
        <span className="text-purple-400 font-mono font-medium">{totalVisible}/{totalFunctions}</span>
      </span>

      {/* 右侧装饰 */}
      <div className="ml-auto flex items-center gap-2 text-gray-500 relative z-10">
        <span className="text-xs font-mono opacity-40 tracking-wider">MathViz Pro</span>
      </div>
    </footer>
  );
};
